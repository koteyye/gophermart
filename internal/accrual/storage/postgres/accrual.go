...
func (a *Accrual) UpdateGoodAccrual(ctx context.Context, goodID uuid.UUID, accrual int) error {
...
func (a *Accrual) GetMatchByName(ctx context.Context, matchName string) (uuid.UUID, error) {
	var matchID uuid.UUID

	err := a.db.QueryRow(ctx, "select id from matches where match_name in ($1)", matchName).Scan(&matchID)
	if err != nil {
		return uuid.Nil, fmt.Errorf("select match err: %w", mapStorageErr(err))
	}
	return matchID, nil
}
